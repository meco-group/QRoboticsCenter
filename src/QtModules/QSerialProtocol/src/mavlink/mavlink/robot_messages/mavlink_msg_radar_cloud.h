...
#define MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN 48
                               ...
                               const int32_t *x, const int32_t *y)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_CRC);
}
                                   ...
                                   const int32_t *x,const int32_t *y)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_RADAR_CLOUD;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_CRC);
}
...
static inline void mavlink_msg_radar_cloud_send(mavlink_channel_t chan, const int32_t *x, const int32_t *y)
{
    ...
    _mav_put_int32_t_array(buf, 0, x, 6);
    _mav_put_int32_t_array(buf, 24, y, 6);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_RADAR_CLOUD, buf, MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_CRC);
#else
    mavlink_radar_cloud_t packet;

    mav_array_memcpy(packet.x, x, sizeof(int32_t)*6);
    mav_array_memcpy(packet.y, y, sizeof(int32_t)*6);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_RADAR_CLOUD, (const char *)&packet, MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_radar_cloud_send_struct(mavlink_channel_t chan, const mavlink_radar_cloud_t* radar_cloud)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_radar_cloud_send(chan, radar_cloud->x, radar_cloud->y);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_RADAR_CLOUD, (const char *)radar_cloud, MAVLINK_MSG_ID_RADAR_CLOUD_MIN_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_LEN, MAVLINK_MSG_ID_RADAR_CLOUD_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_RADAR_CLOUD_LEN? msg->len : MAVLINK_MSG_ID_RADAR_CLOUD_LEN;
        memset(radar_cloud, 0, MAVLINK_MSG_ID_RADAR_CLOUD_LEN);
    memcpy(radar_cloud, _MAV_PAYLOAD(msg), len);
#endif
}
