...
#define MAVLINK_MSG_ID_53_MIN_LEN 40
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_position_send(chan, position->time, position->x, position->y, position->z, position->x_cmd, position->y_cmd, position->z_cmd, position->x_act, position->y_act, position->z_act, position->x_cont, position->y_cont, position->z_cont);
#else
