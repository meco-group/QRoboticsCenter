...
#define MAVLINK_MSG_ID_PARAM_INT_MIN_LEN 15
#define MAVLINK_MSG_ID_10_LEN 15
#define MAVLINK_MSG_ID_10_MIN_LEN 15
                               ...
                               char ID, uint16_t pointer, const char *name, int32_t value)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_PARAM_INT_MIN_LEN, MAVLINK_MSG_ID_PARAM_INT_LEN, MAVLINK_MSG_ID_PARAM_INT_CRC);
}
                                   ...
                                   char ID,uint16_t pointer,const char *name,int32_t value)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_PARAM_INT;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_PARAM_INT_MIN_LEN, MAVLINK_MSG_ID_PARAM_INT_LEN, MAVLINK_MSG_ID_PARAM_INT_CRC);
}
...
static inline void mavlink_msg_param_int_send(mavlink_channel_t chan, char ID, uint16_t pointer, const char *name, int32_t value)
{
    ...
    _mav_put_char(buf, 6, ID);
    _mav_put_char_array(buf, 7, name, 8);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_INT, buf, MAVLINK_MSG_ID_PARAM_INT_MIN_LEN, MAVLINK_MSG_ID_PARAM_INT_LEN, MAVLINK_MSG_ID_PARAM_INT_CRC);
#else
    mavlink_param_int_t packet;
    packet.value = value;
    packet.pointer = pointer;
    packet.ID = ID;
    mav_array_memcpy(packet.name, name, sizeof(char)*8);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_INT, (const char *)&packet, MAVLINK_MSG_ID_PARAM_INT_MIN_LEN, MAVLINK_MSG_ID_PARAM_INT_LEN, MAVLINK_MSG_ID_PARAM_INT_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_param_int_send_struct(mavlink_channel_t chan, const mavlink_param_int_t* param_int)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_param_int_send(chan, param_int->ID, param_int->pointer, param_int->name, param_int->value);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_INT, (const char *)param_int, MAVLINK_MSG_ID_PARAM_INT_MIN_LEN, MAVLINK_MSG_ID_PARAM_INT_LEN, MAVLINK_MSG_ID_PARAM_INT_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_PARAM_INT_LEN? msg->len : MAVLINK_MSG_ID_PARAM_INT_LEN;
        memset(param_int, 0, MAVLINK_MSG_ID_PARAM_INT_LEN);
    memcpy(param_int, _MAV_PAYLOAD(msg), len);
#endif
}
