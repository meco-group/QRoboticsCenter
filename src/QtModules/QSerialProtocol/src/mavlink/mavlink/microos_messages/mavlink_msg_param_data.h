...
#define MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN 43
#define MAVLINK_MSG_ID_13_LEN 43
#define MAVLINK_MSG_ID_13_MIN_LEN 43
                               ...
                               char ID, uint16_t pointer, const char *name, const uint8_t *value)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN, MAVLINK_MSG_ID_PARAM_DATA_LEN, MAVLINK_MSG_ID_PARAM_DATA_CRC);
}
                                   ...
                                   char ID,uint16_t pointer,const char *name,const uint8_t *value)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_PARAM_DATA;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN, MAVLINK_MSG_ID_PARAM_DATA_LEN, MAVLINK_MSG_ID_PARAM_DATA_CRC);
}
...
static inline void mavlink_msg_param_data_send(mavlink_channel_t chan, char ID, uint16_t pointer, const char *name, const uint8_t *value)
{
    ...
    _mav_put_char_array(buf, 3, name, 8);
    _mav_put_uint8_t_array(buf, 11, value, 32);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_DATA, buf, MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN, MAVLINK_MSG_ID_PARAM_DATA_LEN, MAVLINK_MSG_ID_PARAM_DATA_CRC);
#else
    mavlink_param_data_t packet;
    packet.pointer = pointer;
    packet.ID = ID;
    mav_array_memcpy(packet.name, name, sizeof(char)*8);
    mav_array_memcpy(packet.value, value, sizeof(uint8_t)*32);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_DATA, (const char *)&packet, MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN, MAVLINK_MSG_ID_PARAM_DATA_LEN, MAVLINK_MSG_ID_PARAM_DATA_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_param_data_send_struct(mavlink_channel_t chan, const mavlink_param_data_t* param_data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_param_data_send(chan, param_data->ID, param_data->pointer, param_data->name, param_data->value);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARAM_DATA, (const char *)param_data, MAVLINK_MSG_ID_PARAM_DATA_MIN_LEN, MAVLINK_MSG_ID_PARAM_DATA_LEN, MAVLINK_MSG_ID_PARAM_DATA_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_PARAM_DATA_LEN? msg->len : MAVLINK_MSG_ID_PARAM_DATA_LEN;
        memset(param_data, 0, MAVLINK_MSG_ID_PARAM_DATA_LEN);
    memcpy(param_data, _MAV_PAYLOAD(msg), len);
#endif
}
