...
#define MAVLINK_MSG_ID_PARTITION_MIN_LEN 37
#define MAVLINK_MSG_ID_2_LEN 37
#define MAVLINK_MSG_ID_2_MIN_LEN 37
                               ...
                               char ID, uint16_t index, uint8_t size, uint8_t togo, const uint8_t *value)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_PARTITION_MIN_LEN, MAVLINK_MSG_ID_PARTITION_LEN, MAVLINK_MSG_ID_PARTITION_CRC);
}
                                   ...
                                   char ID,uint16_t index,uint8_t size,uint8_t togo,const uint8_t *value)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_PARTITION;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_PARTITION_MIN_LEN, MAVLINK_MSG_ID_PARTITION_LEN, MAVLINK_MSG_ID_PARTITION_CRC);
}
...
static inline void mavlink_msg_partition_send(mavlink_channel_t chan, char ID, uint16_t index, uint8_t size, uint8_t togo, const uint8_t *value)
{
    ...
    _mav_put_uint8_t(buf, 4, togo);
    _mav_put_uint8_t_array(buf, 5, value, 32);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARTITION, buf, MAVLINK_MSG_ID_PARTITION_MIN_LEN, MAVLINK_MSG_ID_PARTITION_LEN, MAVLINK_MSG_ID_PARTITION_CRC);
#else
    mavlink_partition_t packet;
    packet.index = index;
    packet.ID = ID;
    ...
    packet.togo = togo;
    mav_array_memcpy(packet.value, value, sizeof(uint8_t)*32);
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARTITION, (const char *)&packet, MAVLINK_MSG_ID_PARTITION_MIN_LEN, MAVLINK_MSG_ID_PARTITION_LEN, MAVLINK_MSG_ID_PARTITION_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_partition_send_struct(mavlink_channel_t chan, const mavlink_partition_t* partition)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_partition_send(chan, partition->ID, partition->index, partition->size, partition->togo, partition->value);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PARTITION, (const char *)partition, MAVLINK_MSG_ID_PARTITION_MIN_LEN, MAVLINK_MSG_ID_PARTITION_LEN, MAVLINK_MSG_ID_PARTITION_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_PARTITION_LEN? msg->len : MAVLINK_MSG_ID_PARTITION_LEN;
        memset(partition, 0, MAVLINK_MSG_ID_PARTITION_LEN);
    memcpy(partition, _MAV_PAYLOAD(msg), len);
#endif
}
